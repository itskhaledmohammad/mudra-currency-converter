...
import {
  IFixerAPICurrencyResponse,
  IFixerAPISymbolsResponse,
  IAPICurrencyResponse,
} from './symbols.interface';
import { DateTime } from 'luxon';

async function listSymbols(req: express.Request, res: express.Response) {
  const { data }: { data: IFixerAPISymbolsResponse } = await axiosInstance.get(
    `/symbols?access_key=${process.env.FIXER_API_KEY}`,
  );
  return res.status(200).json(data);
}

async function listAllCurrencyRate(req: express.Request, res: express.Response) {
  const { data }: { data: IFixerAPICurrencyResponse } = await axiosInstance.get(
    `/latest?access_key=${process.env.FIXER_API_KEY}`,
  );
  return res.status(200).json(data);
}
